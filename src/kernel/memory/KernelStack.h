#ifndef KERNELSTACK_H
#define KERNELSTACK_H
...
namespace Memory
{
    ...
    {
    public:
        KernelStack(uint8_t* start_address, uint8_t* end_address, bool debug = true)
            : start_address(start_address), end_address(end_address), stack_pointer(start_address), debug(debug) { }

        KernelStack(uint8_t* start_address, int max_item_count, bool debug = true)
